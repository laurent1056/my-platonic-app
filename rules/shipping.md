...
For the current static-first product: relevant `npm run check`,
`npm run validate:data`, and `npm run build` pass; category routes render from
the canonical CSV; the index, `DECLARED`, and `EMPTY` flows work in a browser;
the Oracle remains absent from the public build; images have an explicit source
or fallback; accessible labels and Vercel canonical metadata are present; and
the relevant product docs are updated.
...
- Smoke-test light/dark themes, mobile index browsing, category pages, empty
  states, image fallbacks, root-relative assets, and the absence of `/oracle/`.
